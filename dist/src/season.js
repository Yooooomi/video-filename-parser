...
exports.parseMatchCollection = exports.completeRange = exports.parseSeason = void 0;
...
function parseSeason(title) {
    ...
    let simpleTitle = (0, simplifyTitle_js_1.simplifyTitle)(title);
...
exports.completeRange = completeRange;
const indexOfEnd = (str1, str2) => {
    const io = str1.indexOf(str2);
    return io === -1 ? -1 : io + str2.length;
};
function parseMatchCollection(match, simpleTitle) {
    const { groups } = match;
    if (groups === undefined) {
    ...
    if (airYear < 1900 || Number.isNaN(airYear)) {
        let seasons = [groups['season'], groups['season1']]
            ...
            .map(x => {
            lastSeasonEpisodeStringIndex = Math.max(indexOfEnd(simpleTitle, x ?? ''), lastSeasonEpisodeStringIndex);
            return Number(x);
        });
        if (seasons.length > 1) {
            seasons = (0, exports.completeRange)(seasons);
        }
        ...
        if (absoluteEpisodeCaptures.length > 0) {
            ...
            if (first % 1 !== 0 || last % 1 !== 0) {
                ...
                result.isSpecial = true;
                lastSeasonEpisodeStringIndex = Math.max(indexOfEnd(simpleTitle, absoluteEpisodeCaptures[0] ?? ''), lastSeasonEpisodeStringIndex);
            }
            else {
                const count = last - first + 1;
                // AbsoluteEpisodeNumbers in radarr
                result.episodeNumbers = [...Array(Math.floor(count)).keys()].map(k => k + Math.floor(first));
                if (groups['special']) {
                    result.isSpecial = true;
                }
            }
        ...
        lastSeasonEpisodeStringIndex = Math.max(indexOfEnd(simpleTitle, groups['airyear'] ?? ''), lastSeasonEpisodeStringIndex);
        lastSeasonEpisodeStringIndex = Math.max(indexOfEnd(simpleTitle, groups['airmonth'] ?? ''), lastSeasonEpisodeStringIndex);
        lastSeasonEpisodeStringIndex = Math.max(indexOfEnd(simpleTitle, groups['airday'] ?? ''), lastSeasonEpisodeStringIndex);
...
exports.parseMatchCollection = parseMatchCollection;
