...
for (const [postTitle, title, season] of fullSeasonRelease) {
    (0, vitest_1.it)(`parse full season release "${postTitle}"`, () => {
        const result = (0, index_js_1.parseSeason)(postTitle);
        (0, vitest_1.expect)(result.seasons[0]).toBe(season);
        (0, vitest_1.expect)(result.seriesTitle).toBe(title);
    });
}
const dayEpisodeCases = [
    [
...
for (const [postTitle, title, airDate] of dayEpisodeCases) {
    (0, vitest_1.it)(`parse day season release "${postTitle}"`, () => {
        const result = (0, index_js_1.parseSeason)(postTitle);
        (0, vitest_1.expect)(result.seriesTitle).toBe(title);
        (0, vitest_1.expect)(result.airDate).toEqual(airDate);
        (0, vitest_1.expect)(result.seasons.length).toBe(0);
    });
}
const seasonPackCases = [
    [
...
for (const [postTitle, title, seasons] of seasonPackCases) {
    (0, vitest_1.it)(`parse multi season release "${postTitle}"`, () => {
        const result = (0, index_js_1.parseSeason)(postTitle);
        (0, vitest_1.expect)(result.seasons).toEqual(seasons);
        (0, vitest_1.expect)(result.seriesTitle).toBe(title);
        (0, vitest_1.expect)(result.isMultiSeason).toBe(true);
    });
}
const multiEpisodeCases = [
...
for (const [postTitle, title, episodes] of multiEpisodeCases) {
    (0, vitest_1.it)(`parse multi season release "${postTitle}"`, () => {
        const result = (0, index_js_1.parseSeason)(postTitle);
        (0, vitest_1.expect)(result.episodeNumbers).toEqual(episodes);
        (0, vitest_1.expect)(result.seriesTitle).toBe(title);
        (0, vitest_1.expect)(result.isMultiSeason).toBe(false);
    });
}
const partialSeasonPackCases = [
...
for (const [postTitle, title, season, seasonPart] of partialSeasonPackCases) {
    (0, vitest_1.it)(`parse partial season release "${postTitle}"`, () => {
        const result = (0, index_js_1.parseSeason)(postTitle);
        (0, vitest_1.expect)(result.seasons[0]).toEqual(season);
        (0, vitest_1.expect)(result.seasonPart).toBe(seasonPart);
        (0, vitest_1.expect)(result.seriesTitle).toBe(title);
        (0, vitest_1.expect)(result.isPartialSeason).toBe(true);
    });
}
const crapCases = [
...
for (const [title] of crapCases) {
    (0, vitest_1.it)(`should not parse ${title}`, () => {
        (0, vitest_1.expect)((0, index_js_1.parseSeason)(title)).toBeNull();
    });
}
const oddCases = [
...
for (const [postTitle, title, season, episodeNumber] of oddCases) {
    (0, vitest_1.it)(`should not parse ${postTitle}`, () => {
        const result = (0, index_js_1.parseSeason)(postTitle);
        (0, vitest_1.expect)(result.seriesTitle).toBe(title);
        (0, vitest_1.expect)(result.seasons.length).toBe(1);
        (0, vitest_1.expect)(result.seasons[0]).toBe(season);
        (0, vitest_1.expect)(result.episodeNumbers.length).toBe(1);
        (0, vitest_1.expect)(result.episodeNumbers[0]).toBe(episodeNumber);
    });
}
const animeSpecialCases = [
...
for (const [postTitle, title, episodeNumber] of animeSpecialCases) {
    (0, vitest_1.it)(`parse anime title ${postTitle}`, () => {
        const result = (0, index_js_1.parseSeason)(postTitle);
        (0, vitest_1.expect)(result.seriesTitle).toBe(title);
        (0, vitest_1.expect)(result.episodeNumbers.length).toBe(1);
        (0, vitest_1.expect)(result.episodeNumbers[0]).toBe(episodeNumber);
        (0, vitest_1.expect)(result.isSpecial).toBe(true);
    });
}
const animeRecapCases = [
...
for (const [postTitle, title, specialEpisodeNumber] of animeRecapCases) {
    (0, vitest_1.it)(`handle anime recap episodes ${postTitle}`, () => {
        const result = (0, index_js_1.parseSeason)(postTitle);
        (0, vitest_1.expect)(result.seriesTitle).toBe(title);
        (0, vitest_1.expect)(result.episodeNumbers.length).toBe(1);
        (0, vitest_1.expect)(result.episodeNumbers[0]).toBe(specialEpisodeNumber);
        (0, vitest_1.expect)(result.fullSeason).toBe(false);
    });
}
const seasonSubpackCases = [
...
for (const [postTitle, title, season] of seasonSubpackCases) {
    (0, vitest_1.it)(`parse season subpack ${postTitle}`, () => {
        const result = (0, index_js_1.parseSeason)(postTitle);
        (0, vitest_1.expect)(result.seasons[0]).toBe(season);
        (0, vitest_1.expect)(result.seriesTitle).toBe(title);
        (0, vitest_1.expect)(result.isSeasonExtra).toBe(true);
        (0, vitest_1.expect)(result.fullSeason).toBe(true);
    });
}
const seasonExtraCases = [
...
for (const [postTitle, title, season] of seasonExtraCases) {
    (0, vitest_1.it)(`parse season extras ${postTitle}`, () => {
        const result = (0, index_js_1.parseSeason)(postTitle);
        (0, vitest_1.expect)(result.seasons[0]).toBe(season);
        (0, vitest_1.expect)(result.seriesTitle).toBe(title);
        (0, vitest_1.expect)(result.isSeasonExtra).toBe(true);
...
for (const [postTitle, title, absoluteEpisodeNumber, seasonNumber] of absoluteEpisodeCases) {
    (0, vitest_1.it)(`parse absolute episode numbers ${postTitle}`, () => {
        const result = (0, index_js_1.parseSeason)(postTitle);
        (0, vitest_1.expect)(result.seriesTitle).toBe(title);
        (0, vitest_1.expect)(result.episodeNumbers.length).toBe(1);
        (0, vitest_1.expect)(result.episodeNumbers[0]).toBe(absoluteEpisodeNumber);
        (0, vitest_1.expect)(result.seasons[0] ?? 0).toBe(seasonNumber);
        (0, vitest_1.expect)(result.fullSeason).toBe(false);
    });
}
