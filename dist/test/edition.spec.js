...
for (const [title, result] of cases) {
    (0, vitest_1.it)(`get edition from "${title}"`, () => {
        (0, vitest_1.expect)((0, index_js_1.parseEdition)(title)).toEqual(result);
    });
}
const hardcodedSubsCases = [
...
for (const [title, result] of hardcodedSubsCases) {
    (0, vitest_1.it)(`get hardcoded subs from "${title}"`, () => {
        (0, vitest_1.expect)((0, index_js_1.parseEdition)(title).hardcodedSubs).toEqual(result);
    });
}
