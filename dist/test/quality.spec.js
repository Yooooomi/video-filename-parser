...
for (const [title, result] of versionCases) {
    (0, vitest_1.it)(`parse version "${title}"`, () => {
        (0, vitest_1.expect)((0, index_js_1.parseQualityModifyers)(title).version).toBe(result);
    });
}
const realCases = [
...
for (const [title, result] of realCases) {
    (0, vitest_1.it)(`parse real version "${title}"`, () => {
        (0, vitest_1.expect)((0, index_js_1.parseQualityModifyers)(title).real).toBe(result);
    });
}
const webdl480pCases = [
...
for (const [title, proper] of webdl480pCases) {
    (0, vitest_1.it)(`parse 480p quality "${title}"`, () => {
        const quality = (0, index_js_1.parseQuality)(title);
        (0, vitest_1.expect)(quality.sources[0]).toBe(index_js_1.Source.WEBDL);
        (0, vitest_1.expect)(quality.resolution).toBe(index_js_1.Resolution.R480P);
        (0, vitest_1.expect)(quality.modifier).toBe(null);
        (0, vitest_1.expect)(quality.revision.version).toBe(proper ? 2 : 1);
    });
}
const webdl720pCases = [
...
for (const [title, proper] of webdl720pCases) {
    (0, vitest_1.it)(`parse webdl 720 quality "${title}"`, () => {
        const quality = (0, index_js_1.parseQuality)(title);
        (0, vitest_1.expect)(quality.sources[0]).toBe(index_js_1.Source.WEBDL);
        (0, vitest_1.expect)(quality.resolution).toBe(index_js_1.Resolution.R720P);
        (0, vitest_1.expect)(quality.modifier).toBe(null);
        (0, vitest_1.expect)(quality.revision.version).toBe(proper ? 2 : 1);
    });
}
const webdl1080pCases = [
...
for (const [title, proper] of webdl1080pCases) {
    (0, vitest_1.it)(`parse webdl 1080 quality "${title}"`, () => {
        const quality = (0, index_js_1.parseQuality)(title);
        (0, vitest_1.expect)(quality.sources[0]).toBe(index_js_1.Source.WEBDL);
        (0, vitest_1.expect)(quality.resolution).toBe(index_js_1.Resolution.R1080P);
        (0, vitest_1.expect)(quality.modifier).toBe(null);
        (0, vitest_1.expect)(quality.revision.version).toBe(proper ? 2 : 1);
    });
}
const webrip1080pCases = [
...
for (const [title, proper] of webrip1080pCases) {
    (0, vitest_1.it)(`parse webrip 1080 quality "${title}"`, () => {
        const quality = (0, index_js_1.parseQuality)(title);
        (0, vitest_1.expect)(quality.sources[0]).toBe(index_js_1.Source.WEBRIP);
        (0, vitest_1.expect)(quality.resolution).toBe(index_js_1.Resolution.R1080P);
        (0, vitest_1.expect)(quality.modifier).toBe(null);
        (0, vitest_1.expect)(quality.revision.version).toBe(proper ? 2 : 1);
    });
}
const webdl2160pCases = [
...
for (const [title, proper] of webdl2160pCases) {
    (0, vitest_1.it)(`parse webdl 2160 quality "${title}"`, () => {
        const quality = (0, index_js_1.parseQuality)(title);
        (0, vitest_1.expect)(quality.sources[0]).toBe(index_js_1.Source.WEBDL);
        (0, vitest_1.expect)(quality.resolution).toBe(index_js_1.Resolution.R2160P);
        (0, vitest_1.expect)(quality.modifier).toBe(null);
        (0, vitest_1.expect)(quality.revision.version).toBe(proper ? 2 : 1);
    });
}
const bluray720Cases = [
...
for (const [title, proper] of bluray720Cases) {
    (0, vitest_1.it)(`parse bluray 720p quality "${title}"`, () => {
        const quality = (0, index_js_1.parseQuality)(title);
        (0, vitest_1.expect)(quality.sources[0]).toBe(index_js_1.Source.BLURAY);
        (0, vitest_1.expect)(quality.resolution).toBe(index_js_1.Resolution.R720P);
        (0, vitest_1.expect)(quality.modifier).toBe(null);
        (0, vitest_1.expect)(quality.revision.version).toBe(proper ? 2 : 1);
    });
}
const bluray1080Cases = [
...
for (const [title, proper] of bluray1080Cases) {
    (0, vitest_1.it)(`parse bluray 1080p quality "${title}"`, () => {
        const quality = (0, index_js_1.parseQuality)(title);
        (0, vitest_1.expect)(quality.sources[0]).toBe(index_js_1.Source.BLURAY);
        (0, vitest_1.expect)(quality.resolution).toBe(index_js_1.Resolution.R1080P);
        (0, vitest_1.expect)(quality.modifier).toBe(null);
...
const bluray576pQuality = [
    ['Movie.Name.2004.576p.BDRip.x264-HANDJOB'],
    ['Hannibal.S01E05.576p.BluRay.DD5.1.x264-HiSD'],
];
for (const [title] of bluray576pQuality) {
    (0, vitest_1.it)(`parse bluray 576 quality "${title}"`, () => {
        const quality = (0, index_js_1.parseQuality)(title);
        (0, vitest_1.expect)(quality.sources[0]).toBe(index_js_1.Source.BLURAY);
        (0, vitest_1.expect)(quality.resolution).toBe(index_js_1.Resolution.R576P);
        (0, vitest_1.expect)(quality.modifier).toBe(null);
    });
}
const remux1080pQuality = [
...
for (const [title] of remux1080pQuality) {
    (0, vitest_1.it)(`parse bluray 1080p remux quality "${title}"`, () => {
        const quality = (0, index_js_1.parseQuality)(title);
        (0, vitest_1.expect)(quality.sources[0]).toBe(index_js_1.Source.BLURAY);
        (0, vitest_1.expect)(quality.resolution).toBe(index_js_1.Resolution.R1080P);
        (0, vitest_1.expect)(quality.modifier).toBe(index_js_1.QualityModifier.REMUX);
    });
}
const remux2160pQuality = [
...
for (const [title] of remux2160pQuality) {
    (0, vitest_1.it)(`parse bluray 2160P remux quality "${title}"`, () => {
        const quality = (0, index_js_1.parseQuality)(title);
        (0, vitest_1.expect)(quality.sources[0]).toBe(index_js_1.Source.BLURAY);
        (0, vitest_1.expect)(quality.resolution).toBe(index_js_1.Resolution.R2160P);
        (0, vitest_1.expect)(quality.modifier).toBe(index_js_1.QualityModifier.REMUX);
...
for (const [title, proper] of hdtv720pCases) {
    (0, vitest_1.it)(`parse hdtv 720p quality "${title}"`, () => {
        const quality = (0, index_js_1.parseQuality)(title);
        (0, vitest_1.expect)(quality.sources[0]).toBe(index_js_1.Source.TV);
        (0, vitest_1.expect)(quality.resolution).toBe(index_js_1.Resolution.R720P);
        (0, vitest_1.expect)(quality.modifier).toBe(null);
        (0, vitest_1.expect)(quality.revision.version).toBe(proper ? 2 : 1);
    });
}
const brdisk1080pCases = [
...
for (const [title] of brdisk1080pCases) {
    (0, vitest_1.it)(`parse brdisk 1080p quality "${title}"`, () => {
        const quality = (0, index_js_1.parseQuality)(title);
        (0, vitest_1.expect)(quality.sources[0]).toBe(index_js_1.Source.BLURAY);
        (0, vitest_1.expect)(quality.resolution).toBe(index_js_1.Resolution.R1080P);
        (0, vitest_1.expect)(quality.modifier).toBe(index_js_1.QualityModifier.BRDISK);
    });
}
const rawHdCases = [['Stripes (1981) 1080i HDTV DD5.1 MPEG2-TrollHD']];
for (const [title] of rawHdCases) {
    (0, vitest_1.it)(`parse brdisk 1080p RAWHD quality "${title}"`, () => {
        const quality = (0, index_js_1.parseQuality)(title);
        (0, vitest_1.expect)(quality.sources[0]).toBe(index_js_1.Source.TV);
        (0, vitest_1.expect)(quality.resolution).toBe(index_js_1.Resolution.R1080P);
        (0, vitest_1.expect)(quality.modifier).toBe(index_js_1.QualityModifier.RAWHD);
    });
}
const tsCases = [
...
for (const [title, proper] of tsCases) {
    (0, vitest_1.it)(`parse telesync quality "${title}"`, () => {
        const quality = (0, index_js_1.parseQuality)(title);
        (0, vitest_1.expect)(quality.sources[0]).toBe(index_js_1.Source.TELESYNC);
        (0, vitest_1.expect)(quality.resolution).toBe(index_js_1.Resolution.R720P);
        (0, vitest_1.expect)(quality.modifier).toBe(null);
        (0, vitest_1.expect)(quality.revision.version).toBe(proper ? 2 : 1);
    });
...
for (const [title] of bdripCases) {
    (0, vitest_1.it)(`parse bdrip quality "${title}"`, () => {
        const quality = (0, index_js_1.parseQuality)(title);
        (0, vitest_1.expect)(quality.sources[0]).toBe(index_js_1.Source.BLURAY);
        (0, vitest_1.expect)(quality.resolution).toBe(index_js_1.Resolution.R480P);
        (0, vitest_1.expect)(quality.modifier).toBe(null);
